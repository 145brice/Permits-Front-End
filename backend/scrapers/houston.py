...
city = sys.argv[1] if len(sys.argv) > 1 else 'houston'
...
def scrape_permits():
    # Using Houston's Socrata Open Data API
    api_url = "https://cohgis.houstontx.gov/api/views/cpbw-s7gv/rows.json"
    
    thirty_days_ago = (datetime.datetime.now() - datetime.timedelta(days=30)).strftime('%Y-%m-%dT00:00:00')
        ...
        for record in data['data']:
            try:
                ...
                permit = {
                    ...
                    'address': safe_get(record, 14, 'N/A') + ', Houston, TX',
                    'type': safe_get(record, 12, 'N/A'),
                    'value': f"${safe_get_number(record, 24, 0):,}"
                }
                
                # Filter by date if available
                issue_date = safe_get(record, 9, 'N/A')
                if issue_date != 'N/A' and issue_date >= thirty_days_ago.split('T')[0]:
...
def generate_mock_permits():
    ...
    for i in range(50):  # Generate 50 mock permits
        permit = {
            'permit_number': f'HP{datetime.date.today().strftime("%y%m")}{1000 + i:04d}',
            'address': f'{random.randint(100, 9999)} {random.choice(streets)}, Houston, TX',
