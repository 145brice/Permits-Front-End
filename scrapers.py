...
class SanAntonioScraper(BasePermitScraper):
    """San Antonio, TX - CSV Download"""
    def __init__(self):
        super().__init__("San Antonio")
        self.csv_url = "https://data.sanantonio.gov/dataset/05012dcb-ba1b-4ade-b5f3-7403bc7f52eb/resource/c21106f9-3ef5-4f3a-8604-f992b4db7512/download/permits_issued.csv"

    def scrape(self):
        """Download and parse San Antonio permits CSV"""
        try:
            print("San Antonio: Downloading CSV (may take 30-60 seconds)...")
            ...
            for row in reader:
                ...
                if 'Garage Sale' in permit_type:
                    continue

                permits.append({
                    'contractor_name': row.get('PRIMARY CONTACT') or 'Unknown Contractor',
                    'permit_number': row.get('PERMIT #') or f'SA-{random.randint(10000,99999)}',
                    'address': row.get('ADDRESS') or 'San Antonio, TX',
                    'permit_type': permit_type or 'Building Permit',
                    'value': int(float(row.get('DECLARED VALUATION') or 0)) if row.get('DECLARED VALUATION') else None,
                    'issue_date': issue,
                    'city': self.city_name,
                    'owner_name': row.get('PROJECT NAME') or 'Unknown Owner'
                })
            ...
            if permits:
                print(f"✓ San Antonio: Found {len(permits)} permits from CSV")
                return permits
...
class AtlantaScraper(BasePermitScraper):
    """Atlanta, GA - ArcGIS Feature Server"""
    def __init__(self):
        super().__init__("Atlanta")
        self.api_url = "https://services2.arcgis.com/qvkbeam7Wirps6zC/ArcGIS/rest/services/bseed_building_permits/FeatureServer/0/query"

    def scrape(self):
        """Fetch Atlanta building permits from ArcGIS"""
        try:
            thirty_days_ago_ms = int((datetime.now() - timedelta(days=30)).timestamp() * 1000)

            params = {
                'where': f"issued_date >= {thirty_days_ago_ms}",
                'outFields': '*',
            ...
            if data.get('features'):
                ...
                for feature in data['features']:
                    ...
                    # Parse issued_date (Unix timestamp in milliseconds)
                    issue_ts = attrs.get('issued_date')
                    if issue_ts:
                        try:
                    ...
                    permits.append({
                        'contractor_name': attrs.get('contractor_name') or 'Unknown Contractor',
                        'permit_number': attrs.get('record_id') or f'ATL-{random.randint(10000,99999)}',
                        'address': f"{attrs.get('address') or 'Atlanta'}, GA",
                        'permit_type': attrs.get('permit_type') or 'Building Permit',
                        'value': int(float(attrs.get('valuation') or 0)) if attrs.get('valuation') else None,
                        'issue_date': issue,
                        'city': self.city_name,
                        'owner_name': attrs.get('owner_name') or 'Unknown Owner'
                    })

                if permits:
                    print(f"✓ Atlanta: Found {len(permits)} permits")
                    return permits

        except Exception as e:
            print(f"Atlanta error: {e}")
...
class BostonScraper(BasePermitScraper):
    """Boston, MA - CKAN Datastore API"""
    def __init__(self):
        super().__init__("Boston")
        self.resource_id = "6ddcd912-32a0-43df-9908-63574f8c7e77"
        self.api_url = "https://data.boston.gov/api/3/action/datastore_search"

    def scrape(self):
        """Fetch Boston permits from datastore API"""
        try:
            ...
            if data.get('success') and data.get('result', {}).get('records'):
                ...
                for r in data['result']['records']:
                    ...
                    if issue:
                        try:
                            issue_dt = datetime.fromisoformat(issue.replace('Z', ''))
                            if issue_dt < thirty_days_ago_dt:
                                continue
                            issue = issue_dt.strftime('%Y-%m-%d')
                        except:
                            continue
                    ...
                    permits.append({
                        'contractor_name': r.get('applicant') or 'Unknown Contractor',
                        'permit_number': r.get('permitnumber') or f'BOS-{random.randint(10000,99999)}',
                        'address': f"{r.get('address') or 'Boston'}, MA",
                        'permit_type': r.get('permittypedescr') or r.get('worktype') or 'Building Permit',
                        'value': int(float(str(r.get('declared_valuation', '0')).replace('$','').replace(',',''))) if r.get('declared_valuation') else None,
                        'issue_date': issue,
                        'city': self.city_name,
                        'owner_name': r.get('applicant') or 'Unknown Owner'
                    })

                if permits:
                    print(f"✓ Boston: Found {len(permits)} permits")
                    return permits

        except Exception as e:
            print(f"Boston error: {e}")
...
class MilwaukeeScraper(BasePermitScraper):
    """Milwaukee, WI - CSV Download"""
    def __init__(self):
        super().__init__("Milwaukee")
        self.csv_url = "https://data.milwaukee.gov/dataset/9bada2e0-fad5-4545-8674-1b2c8c4e9f2f/resource/828e9630-d7cb-42e4-960e-964eae916397/download/buildingpermits.csv"

    def scrape(self):
        """Download and parse Milwaukee CSV"""
        try:
            print("Milwaukee: Downloading CSV...")
            ...
            for row in reader:
                # Parse issue date
                issue = row.get('IssueDate') or row.get('IssuedDate') or row.get('issue_date')
                if issue:
                    try:
                        # Try different date formats
                ...
                permits.append({
                    'contractor_name': row.get('ContractorName') or row.get('Contractor') or row.get('contractor_name') or 'Unknown Contractor',
                    'permit_number': row.get('PermitNumber') or row.get('permit_number') or f'MIL-{random.randint(10000,99999)}',
                    'address': f"{row.get('Address') or row.get('address') or 'Milwaukee'}, WI",
                    'permit_type': row.get('PermitType') or row.get('permit_type') or 'Building Permit',
                    'value': int(float(row.get('Valuation') or row.get('Value') or row.get('value') or 0)) if row.get('Valuation') or row.get('Value') or row.get('value') else None,
                    'issue_date': issue,
                    'city': self.city_name,
                    'owner_name': row.get('Owner') or row.get('owner') or 'Unknown Owner'
                })
            ...
            if permits:
                print(f"✓ Milwaukee: Found {len(permits)} permits from CSV")
                return permits

        except Exception as e:
            print(f"Milwaukee error: {e}")
