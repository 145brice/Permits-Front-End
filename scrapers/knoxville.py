#!/usr/bin/env python3
"""
Knoxville Permit Scraper
Data source: City of Knoxville Open Data Portal
"""
...
class KnoxvilleScraper(BaseScraper):
    def __init__(self):
        super().__init__("knoxville")
        # Knoxville uses ArcGIS REST API
        self.base_url = "https://maps.knoxvilletn.gov/server/rest/services/Permits/MapServer/0/query"
        self.permits = []
    ...
    def get_permits(self, max_permits=5000, days_back=90):
        """
        Scrape Knoxville building permits
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("🏗️  Knoxville TN Construction Permits Scraper")
        self.logger.info(f"Fetching up to {max_permits} permits from last {days_back} days...")

        print(f"🏗️  Knoxville TN Construction Permits Scraper")
        print(f"=" * 60)
        ...
        while len(self.permits) < max_permits:
            try:
                ...
                for feature in data['features']:
                    ...
                    if not validate_state(address, 'knoxville', self.logger):
                        continue
        ...
        self.logger.info(f"Successfully collected {len(self.permits)} Knoxville permits")
        print(f"✅ Successfully collected {len(self.permits)} Knoxville permits")
        return self.permits
