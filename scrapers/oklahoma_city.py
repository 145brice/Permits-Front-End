#!/usr/bin/env python3
"""
Oklahoma City Permit Scraper
Data source: City of Oklahoma City Open Data Portal
"""
...
class OklahomaCityScraper(BaseScraper):
    def __init__(self):
        super().__init__("oklahoma_city")
        # Oklahoma City uses ArcGIS REST API
        self.base_url = "https://services6.arcgis.com/7H1Pq4OJQlTHZGbL/arcgis/rest/services/Building_Permits/FeatureServer/0/query"
        self.permits = []
    ...
    def get_permits(self, max_permits=5000, days_back=90):
        """
        Scrape Oklahoma City building permits
        ...
        """
        self.logger.info("=" * 60)
        self.logger.info("🏗️  Oklahoma City OK Construction Permits Scraper")
        self.logger.info(f"Fetching up to {max_permits} permits from last {days_back} days...")

        print(f"🏗️  Oklahoma City OK Construction Permits Scraper")
        print(f"=" * 60)
        ...
        while len(self.permits) < max_permits:
            try:
                ...
                for feature in data['features']:
                    ...
                    # STATE VALIDATION: Only accept Oklahoma addresses
                    if not validate_state(address, 'oklahoma_city', self.logger):
                        continue
        ...
        self.logger.info(f"Successfully collected {len(self.permits)} Oklahoma City permits")
        print(f"✅ Successfully collected {len(self.permits)} Oklahoma City permits")
        return self.permits
